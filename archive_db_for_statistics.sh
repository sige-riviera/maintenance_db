...
psql -d postgres -c "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = '$sourceDb';"

# Create a archived database for statistics on 1st january
if [[ $currentDay=01 && $currentMonth=01 ]]
then
  psql -d postgres -c "CREATE DATABASE qwat_prod_statistics_${currentYear}${currentMonth}${currentDay} WITH TEMPLATE $sourceDb;"
fi
