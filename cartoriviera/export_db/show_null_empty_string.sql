...
rec_num record;
BEGIN
FOR rec_null IN SELECT * FROM information_schema.columns WHERE table_schema = 'sige_qgis_cartoriviera' AND table_name LIKE '%_mn95' AND data_type IN ('character varying','text') LOOP
...
END LOOP;
FOR rec_empty IN SELECT * FROM information_schema.columns WHERE table_schema = 'sige_qgis_cartoriviera' AND table_name LIKE '%_mn95' AND data_type IN ('character varying','text') LOOP
